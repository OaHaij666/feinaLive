...
class EasyVtuberRunner:
    ...
    async def start(self):
        ...
        self._running = True
        logger.info("EasyVtuber 启动完成，Spout2 输出")
    ...
    async def run_loop(self):
        ...
        if getattr(args, 'output_spout2', False):
            if SpoutGL_Sender is None:
                logger.warning("SpoutGL 不可用，Spout2 输出被禁用")
