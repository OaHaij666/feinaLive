...
class THA4StudentORTSessions:
    ...
    def __init__(self, model_dir: str, device_id: int = 0):
        ...
        self.device = 'cpu'
        try:
            test_tensor = ort.OrtValue.ortvalue_from_shape_and_type((1,1), np.float32, 'cuda')
            self.device = 'cuda'
            del test_tensor
            print('THA4 Student Model using CUDA device')
        except Exception as e:
            print(f'THA4 Student Model CUDA not available, using CPU: {e}')
