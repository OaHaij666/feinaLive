...
from apps.live.music.models import MusicItem, MusicStatus, QueueStats
...
class MusicQueue:
    ...
    async def next(self) -> Optional[MusicItem]:
        async with self._lock:
            if self._current:
                ...
                logger.info(f"[Queue.next] 当前歌曲已标记完成: {self._current.title}")
            if self._queue:
                self._current = self._queue.popleft()
                self._current.status = MusicStatus.PLAYING
                self._current.playedAt = datetime.now()
                logger.info(f"[Queue.next] 取出下一首: {self._current.title}, audioUrl={'有' if self._current.audioUrl else '无'}")
                return self._current
            else:
                logger.info("[Queue.next] 队列为空")
                self._current = None
                return None
    ...
    async def count_user_requests(self, user: str) -> int:
        async with self._lock:
            count = 0
            if self._current and self._current.requestedBy == user:
                count += 1
